...
class ClientChatServerTest extends TestKit(ActorSystem("MySpec")) with ImplicitSender
  ...
  "A client when interacts with ChatServer" must {
    ...
    "Send group string messages to its UI according to their ordering" in {
      ...
      client ! SetActorView(view.ref)
      view.expectMsgClass(classOf[UpdateUserAndGroupActive])
      client ! StringMessageFromGroupServer("message1", 1, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 1 => Unit})
      client ! StringMessageFromGroupServer("message2", 3, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 1 => Unit})
      client ! StringMessageFromGroupServer("message4", 5, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 1 => Unit})
      client ! StringMessageFromGroupServer("message3", 2, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 3 => Unit})
      client ! StringMessageFromGroupServer("message2", 4, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 5 => Unit})
    }
    ...
    "Send group messages and group attachments to its UI according to their ordering" in {
      ...
      client ! StringMessageFromGroupServer("message1", 1, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 1 => Unit})
      client ! AttachmentMessageFromGroupServer(attachMentTest, 3, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 1 => Unit})
      client ! StringMessageFromGroupServer("message5", 5, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 1 => Unit})
      client ! AttachmentMessageFromGroupServer(attachMentTest, 2, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 3 => Unit})
      client ! AttachmentMessageFromGroupServer(attachMentTest, 4, "sender", "groupA")
      view.expectMsgPF()({case UpdateStoryComboGroupMessage(msgs, _) if msgs("groupA").length == 5 => Unit})
    }
  }
}
