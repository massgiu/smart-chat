...
class ClientChatGroupServerTest extends TestKit(ActorSystem("MySpec")) with ImplicitSender
  ...
  "ChatGroup Server when interacts with clients" must {
    ...
    "accept a client request to create chatGroup, accept the request of a second Client to join to it and then exchange messages between them" in {
      ...
      clientOne.expectMsg(StringMessageFromServer("groupOneMessage1", 1, clientTwoName, clientOneName))
      clientTwo.expectMsg(StringMessageFromServer("groupOneMessage1", 1, clientTwoName, clientTwoName))
      clientOne.send(chatGroupServerAForClientOne,GroupChatServer.GroupMessage("groupOneMessage2",clientTwoName))
      clientOne.expectMsg(StringMessageFromServer("groupOneMessage2", 2, clientTwoName, clientOneName))
      clientTwo.expectMsg(StringMessageFromServer("groupOneMessage2", 2, clientTwoName, clientTwoName))
      ...
      clientOne.expectMsg(StringMessageFromServer("groupOneMessage3", 3, clientThreeName, clientOneName))
      clientThree.expectMsg(StringMessageFromServer("groupOneMessage3", 3, clientThreeName, clientThreeName))
