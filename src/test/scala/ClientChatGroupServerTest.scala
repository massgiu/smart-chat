...
class ClientChatGroupServerTest extends TestKit(ActorSystem("MySpec")) with ImplicitSender
  ...
  "ChatGroup Server when interacts with clients" must {
    ...
    "accept a client request to create chatGroup, accept the request of a second Client to join to it and then exchange messages between them" in {
      ...
      clientOne.expectMsg(StringMessageFromGroupServer("groupOneMessage1", 1, clientTwoName, groupNameOne))
      clientTwo.expectMsg(StringMessageFromGroupServer("groupOneMessage1", 1, clientTwoName, groupNameOne))
      clientOne.send(chatGroupServerAForClientOne,GroupChatServer.GroupMessage("groupOneMessage2",clientTwoName))
      clientOne.expectMsg(StringMessageFromGroupServer("groupOneMessage2", 2, clientTwoName, groupNameOne))
      clientTwo.expectMsg(StringMessageFromGroupServer("groupOneMessage2", 2, clientTwoName, groupNameOne))
      ...
      clientOne.expectMsg(StringMessageFromGroupServer("groupOneMessage3", 3, clientThreeName, groupNameOne))
      clientThree.expectMsg(StringMessageFromGroupServer("groupOneMessage3", 3, clientThreeName, groupNameOne))
